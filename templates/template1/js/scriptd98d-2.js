(function () {
    ...
    function postToGoogleForms(data) {
        const formURL = "https://docs.google.com/forms/u/0/d/e/1FAIpQLSen-oEb1sEMy9Ydj8_DpFurFKqLKb9MUIpk5y97otdiYZ87kw/formResponse";
        const formData = new FormData();
        formData.append('entry.586716902', data.name); // Họ tên
        formData.append('entry.761970245', data.phone); // Số điện thoại
        formData.append('entry.622267091', data.content); // Lời chúc

        fetch(formURL, {
            method: "POST",
            body: formData
        }).then(() => {
            toastr.success('Cảm ơn bạn đã gửi lời chúc!');
            $("#wish-form")[0].reset();
        }).catch((error) => {
            toastr.success('Cảm ơn bạn đã gửi lời chúc!');
            $("#wish-form")[0].reset();
